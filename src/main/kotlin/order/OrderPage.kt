...
object OrderPage{
    ...
    fun order(user: User, items: List<CartItem>) {
        if (items.isEmpty()) {
        ...
        if (user.point > 0) {
            ...
            when (readLine()?.uppercase()) {
                "Y" -> {
                    userPoint = if (user.point >= totalAmount) totalAmount else user.point
